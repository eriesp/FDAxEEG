...
for i,indx in enumerate(indxs):
    ...
    colnames.append('int_z'+str(zona)+'_b'+str(banda))
...
for i,indx in enumerate(indxs):
    ...
    print('Zona: '+str(zona)+'\tBanda: '+str(banda))
    colnames.append('norm_z'+str(zona)+'_b'+str(banda))

mat_norm_adhd = pd.DataFrame(matrice_norm_adhd)
mat_norm_cont = pd.DataFrame(matrice_norm_cont)

# save matrices
# mat_norm_adhd.to_csv(home_path+'\Bonf_Feat_norm_adhd.csv', index = False, sep = ',')
# mat_norm_cont.to_csv(home_path+'\Bonf_Feat_norm_cont.csv', index = False, sep = ',')
...
for i,indx in enumerate(indxs):
    ...
    print('Zona: '+str(zona)+'\tBanda: '+str(banda))
    colnames.append('max_z'+str(zona)+'_b'+str(banda))

matrice_picco_adhd = pd.DataFrame(matrice_picco_adhd)
matrice_picco_cont = pd.DataFrame(matrice_picco_cont)

# save matrices
# matrice_picco_adhd.to_csv(home_path+'\Bonf_Feat_picco_adhd.csv', index = False, sep = ',')
# matrice_picco_cont.to_csv(home_path+'\Bonf_Feat_picco_cont.csv', index = False, sep = ',')


df_int = pd.concat([mat_int_adhd, mat_int_cont])
df_norm = pd.concat([mat_norm_adhd, mat_norm_cont])
df_max = pd.concat([matrice_picco_adhd, matrice_picco_cont])

df = pd.concat([df_int, df_norm, df_max], axis=1)
