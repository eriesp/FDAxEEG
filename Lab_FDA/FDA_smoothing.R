...
# there is strong dependence within two consecutive points, that's why we want to use functions 

setwd("~/UNI/APPLIED STATISTICS/LABORATORI/Lab_FDA")
...
rappincX2 <- ((Xobs0[3:NT]-Xobs0[2:(NT-1)])/(abscissa[3:NT]-abscissa[2:(NT-1)])-
                (Xobs0[2:(NT-1)]-Xobs0[1:(NT-2)])/(abscissa[2:(NT-1)]-
                abscissa[1:(NT-2)]))*2/(abscissa[3:(NT)]-abscissa[1:(NT-2)])
...
# a lot of noise in the derivatives
...
# we want to smooth the derivatives
...
plot(basis)    # linee tratteggiate sono i knots
...
# alternative code 
help(smooth.basis)
Xsp <- smooth.basis(argvals=abscissa, y=Xobs0, fdParobj=basis) # you can use this also to include penalization
...
# much smoother behavior but it's too much -> missing some characteristics
# -> underfitting the curve 
...
functionalPar <- fdPar(fdobj=basis, Lfdobj=3, lambda=1e-8)  # I want to penalize the third derivative
...
# the third field is the penalization term 
...
df
# [1] 17.08055
# there are based on linear operator (kind of it's dimension) (not orthog projection)
...
# want the functions positive -> take a trasformation e.g. exponential
...
basismat1 <- eval.basis(evalarg = growth$age, basisobj = heightbasis12,Lfdobj=1)
